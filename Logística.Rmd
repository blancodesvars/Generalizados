...
Ahora bien, la variable respuesta del modelo de regresión logística multinomial es una variable aleatoria con distribución multinomial, que se puede considerar como el número de éxitos en cada una de las _g_ categorías que se presentan en _n_ ensayos independientes, y su función de distribución viene dada por (Feller, 1967).  
...
Como se mencionó anteriormente para la estimación de los parámetros del modelo se utiliza el hecho de que la distribución multinomial pertenece a la familia exponencial, lo cual se puede demostrar de forma sencilla. Supóngase que ; _Fy_ y,$\theta$ es una función de distribución dependiendo de un único parámetro$\theta$. Se dice que pertenece a la familia exponencial si su función de densidad (o de masa, en el caso discreto) puede expresarse de la forma: 


CORREGIR ESTA FORMULA
...
fitted(test1)
```
En la salida anterior, lo primero que vemos es la llamada, esta es R, que nos recuerda quÃ© modelo ejecutamos, quÃ© opciones especificamos, etc.

A continuaciÃ³n vemos los residuos de desviaciÃ³n, que son una medida del ajuste del modelo. Esta parte de la salida muestra la distribuciÃ³n de los residuos de desviaciÃ³n para los casos individuales utilizados en el modelo. A continuaciÃ³n, discutimos cÃ³mo usar los resÃºmenes de la estadÃ?stica de desviaciÃ³n para evaluar el ajuste del modelo.

La siguiente parte de la salida muestra los coeficientes, sus errores estÃ¡ndar, la estadÃ?stica z (a veces denominada estadÃ?stica z de Wald) y los valores p asociados. Ambos Sexo y los dos niveles de edad son estadÃ?sticamente significativos. Los coeficientes de regresiÃ³n logÃ?stica proporcionan el cambio en las probabilidades de registro del resultado para un aumento de una unidad en la variable predictiva.
